...
class PurchaseFlow
{
    ...
    {
        //edit by gzy 最初的计算金额的地方
        if ($itemHolder instanceof Order) {
            $total = 0;
            $CustomerShops = $itemHolder->getCustomer()->getCustomerShops();
            if(count($CustomerShops) > 0 && $CustomerShops[0]->getStatus() == "Y"){
                $CustomerShop = $CustomerShops[0];
                $discount = $CustomerShop->getCustomerShopLevel()->getDiscount();
                foreach ($itemHolder->getItems() as $item) {
                    if($item->isProduct() && $item->getProduct()->getShopDiscount() == "Y"){
