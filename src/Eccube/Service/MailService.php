...
class MailService
{
     ...
     *
     * @param $Customer 会員情報
     * @param string $文本 アクティベート用url
     */
    public function sendCustomerShopConfirmMailYes(\Eccube\Entity\Customer $Customer)
    {
        log_info('审核店铺送信開始');

        $MailTemplate = $this->mailTemplateRepository->find($this->eccubeConfig['eccube_entry_shop_confirm_yes_mail_template_id']);
        ...
        $this->eventDispatcher->dispatch(EccubeEvents::MAIL_CUSTOMER_SHOP_CONFIRM, $event);

        $count = $this->mailer->send($message, $failures);

        log_info('审核送信完了', ['count' => $count]);
     ...
     * @param string $文本 アクティベート用url
     */
    public function sendCustomerShopConfirmMailNo(\Eccube\Entity\Customer $Customer)
    {
        log_info('审核店铺送信開始');

        $MailTemplate = $this->mailTemplateRepository->find($this->eccubeConfig['eccube_entry_shop_confirm_no_mail_template_id']);
     ...
     * @param string $文本 アクティベート用url
     */
    public function sendEntryShopSubmitMail(\Eccube\Entity\Customer $Customer)
    {
        log_info('审核店铺送信開始');

        $MailTemplate = $this->mailTemplateRepository->find($this->eccubeConfig['eccube_entry_shop_submit_mail_template_id']);
