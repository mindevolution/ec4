...
class OrderHelper
{
    ...
    public function createPurchaseProcessingOrder(Cart $Cart, Customer $Customer)
    {
        ...
        foreach ($OrderItemsGroupBySaleType as $OrderItems) {
            ...
            $this->setDefaultDelivery($Shipping, $Customer);
            $this->entityManager->persist($Shipping);
            $Order->addShipping($Shipping);
        }

        $this->setDefaultPayment($Order, $Customer);
     ...
     */
    protected function setDefaultDelivery(Shipping $Shipping, Customer $Customer)
    {
        // 配送商品に含まれる販売種別を抽出.
        ...
        // edit by gzy 运输方式选择，商铺根据shop关键词，来判断是否展示物流方式
        $is_shop = false;
        $Deliveries_new = [];
        ...
        foreach ($Deliveries as $Deli) {
            ...
            $Delivery_name = $Deli->getName();
            if($is_shop && strstr($Delivery_name, 'SHOP')){
                $Deliveries_new[] = $Deli;
            }
            if(!$is_shop && !strstr($Delivery_name, 'SHOP')){
                $Deliveries_new[] = $Deli;
        ...
        // 初期の配送業者を設定
        $Delivery = current($Deliveries_new);
     ...
     * @param Order $Order
     */
    protected function setDefaultPayment(Order $Order, Customer $Customer)
    {
        ...
        // 利用可能な支払い方法を抽出.
        $Payments = $this->paymentRepository->findAllowedPayments($Deliveries_new, true);
