...
class ShoppingController extends AbstractShoppingController
{
    ...
    public function index(PurchaseFlow $cartPurchaseFlow)
    {
        ...
        if($CustomerShop && $CustomerShop->getStatus() == "Y"){
            ...
            if($nowShop >= $expDateShop){//如果过期时间到了，现根据money判断现在等级，再把money清零
                $money = $CustomerShop->getMoney();
        ...
        //根据CustomerLevel里面的过期时间，如果过期，money清零，level变成blue
        $CustomerLevel = $this->customerLevelRepository->getCustomerLevelByCustomer($this->getUser());
        $expDateLevel = $CustomerLevel->getExpTime();
        if($now >= $expDateLevel){
            $CustomerLevel->setMoney(0);
            $CustomerLevelDetail = $this->customerLevelDetailRepository->findOneBy(['level' => 'BLUE']);
            $CustomerLevel->setCustomerLevelDetail($CustomerLevelDetail);
            $CustomerLevel->setLevel('BLUE');
        ...
        // echo($Order->getPayment()->getId());
        // return;
        // 集計処理.
        log_info('[注文手続] 集計処理を開始します.', [$Order->getId()]);
        $flowResult = $this->executePurchaseFlow($Order, false);

        // 如果是商户会员，且商品有非shop商品时候，利用order,单独计算一个total，来作为前端显示的total数据。
        $total = 0;
        if($CustomerShop && $CustomerShop->getStatus() == "Y"){//判断是商品会员
            foreach($Order->getItems() as $OrderItem){
                // 判断orderitem里面的商品是否是shop商品
                if($OrderItem->isProduct()){//如果里面的内容是商品
                    if($OrderItem->getProduct()->getShopDiscount()== 'Y'){
                        $total += $OrderItem->getPriceIncTax()* $OrderItem->getQuantity() * (100-$CustomerShop->getCustomerShopLevel()->getDiscount())/100.0;
                    }else{//判断不是会员商品的时候
