        ...
        if ($form->isSubmitted() && $form->isValid()) {
            switch ($request->get('mode')) {
                ...
                case 'complete':
                    ...
                    //edit by gzy 注册会员，设置等级为BLANK，设置过期时间为1年
                    $customerLevel = new CustomerLevel();
                    $CustomerLevelDetail = $this->customerLevelDetailRepository->findOneBy(['level' => 'BLANK']);

                    $customerLevel->setCustomer($Customer);
                    $customerLevel->setUpdateDate(new \DateTime());
                    $customerLevel->setCreateDate(new \DateTime());
                    $customerLevel->setLevel($CustomerLevelDetail->getLevel());
