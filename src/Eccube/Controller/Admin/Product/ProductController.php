...
class ProductController extends AbstractController
{
   ...
   */
   function watermarker($srcfile,$waterfile,$pct=100,$position=1){
       //根据MIME类型和路径完成原图和水印图片资源的创建
       $src_mime=getimagesize($srcfile)['mime'];
      $wat_mime=getimagesize($waterfile)['mime'];

    //   $src=$GLOBALS['func'][$src_mime]($srcfile);imagecreatefrompng($file)
    ...
    }elseif($src_mime =='image/gif'){
        $src=imagecreatefromgif($srcfile);
    };
    $wat=imagecreatefrompng($waterfile);
      ...
      imagecopyresized($new_logo, $wat,0, 0,0, 0,$nw, $nh, $wat_w, $wat_h );
      ...
      //生成水印并返回结果 (上传图片，水印，位置，水印宽，水印高，透明度0~100)
      imagecopymerge($src, $new_logo, $src_x, $src_y, 0, 0, $nw, $nh, $pct);
    ...
    public function addImage(Request $request)
    {
        ...
        foreach($files as $file){
            $srcfile=$this->eccubeConfig['eccube_temp_image_dir'].'/'.$file;  //原图
            // $waterfile=$this->eccubeConfig['eccube_save_image_dir'].'/logo.png';  //水印图片
            $waterfile=$_SERVER['DOCUMENT_ROOT'].'/html/template/admin/assets/img/logo.png';  //水印图片
            $mime=getimagesize($srcfile)['mime'];   //获取原图MIME类型 
            $wat=$this->watermarker($srcfile, $waterfile,5,3);  //生成带有水印的图片
            if($mime=='image/jpeg'){
                $flag = imagejpeg($wat,$this->eccubeConfig['eccube_temp_image_dir'].'/'.$file);
            }elseif($mime =='image/png'){
                $flag = imagepng($wat,$this->eccubeConfig['eccube_temp_image_dir'].'/'.$file);
            }elseif($mime =='image/gif'){
                $flag = imagegif($wat,$this->eccubeConfig['eccube_temp_image_dir'].'/'.$file);
            };
            $flag = imagejpeg($wat,$this->eccubeConfig['eccube_temp_image_dir'].$file);
    ...
    /**
     * @Route("/%eccube_admin_route%/product/{id}/yes", requirements={"id" = "\d+"}, name="admin_customer_shop_product_yes")
     */
    ...
    /**
     * @Route("/%eccube_admin_route%/product/{id}/no", requirements={"id" = "\d+"}, name="admin_customer_shop_product_no")
     */
     ...
     * Bulk public action
     *
     * @Route("/%eccube_admin_route%/product/bulk/no", name="admin_customer_shop_products_no", methods={"POST"})
     *
     * @param Request $request
     *
     * @return RedirectResponse
     */
    public function bulk_no(Request $request)
    {
        $this->isTokenValid();
     ...
     * Bulk public action
     *
     * @Route("/%eccube_admin_route%/product/bulk/yes", name="admin_customer_shop_products_yes", methods={"POST"})
     *
     * @param Request $request
     *
     * @return RedirectResponse
     */
    public function bulk_yes(Request $request)
    {
        $this->isTokenValid();
